...
var config = {
    ...
    // 默认菜单配置
    menus: ['undo', 'redo', 'head', 'bold', 'fontSize', 'fontName', 'italic', 'underline', 'strikeThrough', 'foreColor', 'backColor', 'link', 'list', 'justify', 'quote', 'emoticon', 'image', 'table', 'video', 'source', 'fullscreen'],
...
Source.prototype = {
    ...
    _createSource: function _createSource() {
        ...
        var editor = this.editor;
        var $source = $('<textarea class="w-e-source-container"></textarea>');
        this.$parent.append($source);
        this.$source = this.$parent.find('.w-e-source-container');
        this.$source.on('blur', function () {
            var source = _this.$source.val();
            editor.txt.html(source);
            editor.onblur && editor.onblur();
        });
        this.$source.on('focus', function () {
            editor.onfocus && editor.onfocus();
        });
    },

    // 展示源码框
    _showSource: function _showSource() {
        var editor = this.editor;
        var source = editor.txt.html();
